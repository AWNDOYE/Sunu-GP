...
const ProductCard = () => {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (
        ...
        product.colisPriceByLitre === 0
      ) {
        alert(
          "Le prix par litre doit être spécifié pour les produits liquides"
        );
        return;
      }
      // Vérifier la concordance entre les produits liquides et le prix par L
      if (
        product.colisTypeName !== "Produits Liquides" &&
        product.colisPriceByKG === 0
      ) {
        alert(
          "Le prix par Kg doit être spécifié pour tous les produits exceptés les liquides"
        );
        return;
      }
  ...
  const handleDelete = async () => {
    try {
      const response = await axios
        .delete(`${Config.api_url}/deleteColis/${productId}`)
        .then((response) => {
  ...
  return (
    <>
      <Card className="card cardChangeProduct">
        <CardTitle className="tittleCard">MODIFICATION PRODUIT</CardTitle>
        <Form>
          <CardBody className="card-body cardChangeBody">
            <div>
              <CardImg
            ...
            <div style={{width:"50rem"}}>
              <FormGroup className="form-group">
                ...
                >
                  ...
                  <option value="Courrier - Enveloppe - A4">
                    Courrier - Enveloppe - A4
                  </option>
              ...
              <FormGroup className="form-group">
                ...
                <FormControl
                  ...
                  name="colisImage"
                  onChange={(e) =>
                    setProduct({ ...product, colisImage: e.target.files[0] })
                  }
                  required
                />
              </FormGroup>
