...
export default function TrajetUpdate() {
  ...
  const handleUpdateTrajet = async () => {
    ...
    try {
      ...
      if (trajet.trajet_zonePrice === 0) {
        alert("Veuillez saisir le prix du correspondant au trajet défini.");
      ...
      const response = await axios
        ...
        .then((response) => {
          ...
          alert(
            `Le trajet ${trajet.trajet_PlaceDepartureName} - ${
              trajet.trajet_PlaceArrivalName
            } du ${formatISODate(
              trajet.trajet_DateArrivee
  ...
  const handleDelete = async () => {
    try {
      const response = await axios
        .delete(`${Config.api_url}/deleteTrajet/${trajetId}`)
        .then((response) => {
        ...
        <Form onSubmit={handleSubmit}>
          <CardBody className="card-body cardChangeBody notAlignItems">
            <div style={{ width: "40rem" }}>
              ...
              <Button className="btnShowUsers" onClick={handleShowUsersTrajet}>
                Afficher la liste des utilisateurs du trajet N° {trajet._id}
              </Button>
            </div>
            <div style={{ width: "40rem" }}>
              <CardSubtitle className="infoAuteurTrajet">
